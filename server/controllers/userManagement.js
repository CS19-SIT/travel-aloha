...
exports.deleteUsers = function(req,res) {
    let user_id = req.params.id;
    let getImageQuery = 'SELECT profile_picture from `users` WHERE user_id = "' + user_id + '"';
        ...
        if (result.length > 0) {
            ...
            if (uploadedFile.mimetype === 'image/png' || uploadedFile.mimetype === 'image/jpeg' || uploadedFile.mimetype === 'image/gif') {
                ...
                uploadedFile.mv(`server/uploads/${image_name}`, (err ) => {
                    ...
                    let query = "INSERT INTO `users` (firstname, lastname, gender, birth_date, phoneNumber, userRole, email, profile_picture, username) VALUES ('" +
                        firstname + "', '" + lastname + "', '" + gender + "', '" + birth_date + "', '" + phoneNumber + "', '" + userRole + "', '" + email + "', '" + image_name + "', '" + username + "')";
                    db.query(query, (err, result) => {
                        if (err) {
                            return res.status(500).send(err);
                        }
...
exports.editUsers = function(req,res) {
    ...
    let phoneNumber = req.body.phoneNumber;
    let email = req.body.email;
    let query = "UPDATE `users` SET `firstname` = '" + firstname + "', `lastname` = '" + lastname + "', `gender` = '" + gender + "', `phoneNumber` = '" + phoneNumber + "', `email` = '" + email + "' WHERE `users`.`user_id` = '" + user_id + "'";
    db.query(query, (err, result) => {
        if (err) {
            return res.status(500).send(err);
        }
